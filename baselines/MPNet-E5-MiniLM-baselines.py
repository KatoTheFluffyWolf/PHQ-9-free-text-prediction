...
def encode_students(df_sub, model, batch_size=64, normalize=False, show_progress=False):
    """
    Returns x of shape (n_samples, 9, emb_dim) from raw text using a SentenceTransformer.
    ...
    # Flatten to (n*9,) so we do ONE encode call (faster + consistent)
    ...
    x = embs.reshape(n, len(TEXT_COLS), -1).astype(np.float32)  # (n, 9, emb_dim)
    return x
...
# =========================
SENT_MODEL_NAME = "intfloat/e5-base-v2"  #replace with the model ids below for other domain-general baselines compared in the paper
#"sentence-transformers/all-mpnet-base-v2"
#"sentence-transformers/all-MiniLM-L6-v2"
#intfloat/e5-base-v2

device = "cuda" if torch.cuda.is_available() else "cpu"
st_model = SentenceTransformer(SENT_MODEL_NAME, device=device)
print("Loaded sentence-transformer:", SENT_MODEL_NAME, "| device:", device)
...
for fold, (train_idx, test_idx) in enumerate(skf.split(np.zeros(len(sev)), sev), start=1):
    ...
    # ---------
    x_train = encode_students(df_train_students, st_model, batch_size=64, normalize=False)
    x_test  = encode_students(df_test_students,  st_model, batch_size=64, normalize=False)
