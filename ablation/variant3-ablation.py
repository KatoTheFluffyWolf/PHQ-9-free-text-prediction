...
def encode_students_meanpool(sbert_model, df_sub, batch_size=64):
    """
    Faster: 1 encode call total.
    Returns x of shape (n_samples, emb_dim) by encoding all 9 responses then mean pooling.
    """
    # (n, 9) strings
    ...
    n = arr.shape[0]
    flat_texts = arr.reshape(-1).tolist()  # (n*9,)
    ...
    x_seq = embs.reshape(n, len(TEXT_COLS), -1).astype(np.float32)  # (n, 9, dim)
    return x_seq.mean(axis=1).astype(np.float32)                    # (n, dim)
...
for fold, (train_idx, test_idx) in enumerate(skf.split(np.zeros(len(sev)), sev), start=1):
   ...
   # ---------
    ...
    # ---------
    x_train = encode_students_meanpool(sbert_model, df_train_students)  # (n, dim)
    x_test  = encode_students_meanpool(sbert_model, df_test_students)   # (n, dim)
    ...
    reg_model = build_mlp_regressor(input_dim=emb_dim)
